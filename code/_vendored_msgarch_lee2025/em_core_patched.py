...
em_core_patched.py
==================
ADAPTED COPY of upstream `em_core.py` from
https://github.com/Im-Hyeon-Lee/Day-dependent-Markov-switching-GARCH-model
(commit c84bc297d11e430cd459a20919fee1a425e1dd41).

Upstream license: MIT (see ./LICENSE in this directory).

DIVERGENCE FROM UPSTREAM
------------------------
The upstream `em_core.py` (preserved verbatim alongside this file) contains
a single-character indentation bug at line 202 of the function
`em_fit_ms_garch`:

    upstream:
         for it in range(max_iter):    # 5-space indent -- IndentationError

    patched:
        for it in range(max_iter):     # 4-space indent (matches function body)

The bug prevents Python from parsing the module at all, so we cannot import
upstream's `em_core` directly.  This file contains an otherwise verbatim
copy with that one whitespace fix.  All algorithm logic is unchanged.

Patch metadata for the audit harness:
    PATCHED_LINES = [202]
    DIVERGENCE_BYTES = 1
    DIVERGENCE_KIND  = "whitespace_only_bugfix"

The verbatim file is retained at `./em_core.py` for reference / faithful
representation of the upstream publication state.  Audit harness verifies
both: `em_core.py` matches upstream byte-for-byte; `em_core_patched.py`
differs from upstream by exactly the patch declared above.
"""
...
def em_fit_ms_garch(returns, dows, lam_scad, K=2, max_iter=250, tol=1e-5, verbose=False):
    ...
    for it in range(max_iter):  # PATCH: upstream had 5-space indent (IndentationError); fixed to 4.
