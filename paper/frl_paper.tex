...
of fund-level $\FridayShift$ with size, Treasury status, and turnover
are weak and statistically non-significant under the NAV proxy
($|\rho| \le 0.51$ for all six predictors, only Treasury reaches
$p < 0.05$ and with the opposite sign of the synthetic-mechanism
prior).  We therefore report the wrapper-level Friday-clustering and
its cross-fund asymmetry as a stylized fact that survives an
open-data replication, while \emph{retracting} the
``size/Treasury/flow predicts $\FridayShift$'' cross-sectional claim
pending a non-proxy NAV pipeline.  The roadmap to a flow-measured
...
the max-side average is $27.0\%$ (range $24.4\%$--$29.1\%$), while
the min-side average is $23.6\%$ (range $20.8\%$--$27.2\%$). The
...
Under the 5-day-MA NAV proxy and a forward-filter-only
$\FridayShift$ estimate (\S\,\ref{sec:msgarch}), cross-sectional
Spearman rank correlations between $\FridayShift$ and end-2025
fund characteristics are: $\log\!\mathrm{AUM}$ $\rho = +0.20$
($p = 0.43$, $n = 17$); Treasury-benchmark indicator $\rho = -0.51$
($p = 0.04$); $\log(\mathrm{ADV}/\mathrm{AUM})$ $\rho = -0.26$
($p = 0.32$); bid-ask spread $\rho \approx 0$ ($p = 0.99$);
expense ratio $\rho = -0.38$ ($p = 0.14$). \emph{Note that the
size-positive / Treasury-positive cross-section reported in earlier
drafts is not reproduced by this open-data pipeline}; the
Treasury-coefficient sign in particular reverses relative to the
synthetic-mechanism prior of Lee~\citep{Lee2025}.  We attribute the
divergence to two compounding sources: (i)~the NAV proxy washes out
exactly the close-vs-NAV gap that the $\FridayShift$ ranking is meant
to capture, and (ii)~the vendored MSGARCH backend uses the forward
filter only (no M-step iteration; the full Im-Hyeon-Lee EM is
referenced in \S\,\ref{sec:msgarch} but not yet vendored).  With
residual $\mathrm{df} = 11$, the multivariate Wald $F$ for the
flow-block is $F = 1.34$ ($p > 0.2$).  We therefore retract the
``size and Treasury status predict $\FridayShift$'' claim until a
non-proxy NAV pipeline is implemented.
...
\subsection{T+1 transition (deferred)}
A pre/post stratification around the 28~May 2024 U.S.\ T+1 settlement
transition \citep{SEC2023T1,BessembinderMaxwell2024} is not computed
in this iteration of the pipeline; the underlying $\FridayShift$
estimate is too noisy under the 5-day-MA proxy + forward-filter EM
to support a meaningful event-study stratification.  The full
difference-in-differences with Canadian-ETF synthetic controls
remains future work (Appendix~E.5).
